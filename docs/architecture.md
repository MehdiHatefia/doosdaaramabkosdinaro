...
صفحه `AdDetailView` در مسیر `/advertisements/:id` جزئیات کامل یک آگهی تاییدشده را از `AdvertisementController.show` دریافت می‌کند. ساختار جدید دو ستونه است: ستون اصلی شامل سربرگ معامله، کارت اعتبار بانکی با لوگوی شفاف بانک، مبلغ و بج‌های امنیتی، کارت کهربایی `Safety Tips Card` با چهار نکته اختصاصی انتقال امتیاز و لینک گزارش تخلف، گرید چهارخانه مشخصات و توضیحات است؛ ستون کناری sticky شامل CTA تماس، وضعیت احراز هویت آگهی‌دهنده، نشان/اشتراک‌گذاری، محدودهٔ نقشه و نکات ایمنی بانک است. اطلاعات تماس فقط پس از ورود و احراز کاربر نمایش داده می‌شود و کاربر مهمان به `AuthModal` هدایت می‌شود. پیشنهادهای مرتبط با `bank_id` در پایین صفحه با گرید سه‌ستونه و `AdCard` استاندارد بارگذاری می‌شوند.

مبالغ در `AdDetailView` با `Intl.NumberFormat('fa-IR')` جداکننده‌گذاری می‌شوند. `formatMillionMoney` برای مقادیر مدل که بر حسب میلیون تومان ذخیره شده‌اند و `formatCompactMoney` برای کارت اعتبار بانکی استفاده می‌شود تا از نمایش مبهمی مثل `50000000 میلیون تومان` جلوگیری شود؛ اقساط و کارمزد نیز در همان گرید با واحد خوانا نمایش داده می‌شوند.
